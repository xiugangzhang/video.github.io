    ...
    xframe.prototype.init = function (selector, context) {
        ...
        if (typeof  selector === 'string') {
            if (document.querySelectorAll) {
                ...
                // 获取所有元素节点
                var nodeList = IESelectorAll(selector);
                this.length = nodeList.length;
                for (var i = 0, len = this.length; i < len; i++) {
                    this[i] = nodeList[i];
                }

                function IESelectorAll(selector) {
        ...
        */
        ...
        ajax: function (opt) {
            ...
            if (opt.type === 'POST') {
                ...
                opt.error('req type error!');
            }

            // 添加一个事件
            xmlHttp.onreadystatechange = function () {
                if (xmlHttp.readyState === 4 && xmlHttp.status === 200) {
                    // 本次请求成功， 在看一下用户需要的数据类型
                    if (opt.dataType === 'json'){
                        var json = JSON.parse(xmlHttp.responseText);
                        return opt.success(json);
                    }
                    // 默认返回的是一个字符串
...
;(function (xframe) {
    ...
    xframe.extend({
        ...
        hasClass: function (val) {
            ...
            // 默认只会获取第一个元素的相关信息
            var items = this[0].className.trim().split(' ');
            if (items.length > 1) {
                var i = 0,
                    len = items.length;
                for (; i < len; i++){
                ...
                if (arr.length >= 2) {
                    ...
                    for (; i < len; i++) {
                        if (arr[i].trim() === val.trim()) {
                            this.className = this.className.replace(val.trim(), '');
                            return;
                        }
                    }
                }
...
;(function (xframe) {
    ...
    xframe.Animate = (function (xframe) {
        ...
        api.move = function (obj) {
            ...
            var tween = api.getTween(obj.now, obj.pass, obj.duration, obj.type);
        ...
        api.add = function () {
            ...
            try {
                // 1. 调用适配器准备参数
                api.adapterMany(id, json, duration, type);
         ...
         * @param duration
         */
        api.adapterOne = function (id, json, duration, type) {
         ...
         * @param data
         */
        api.adapterMany = function (id, json, data, type) {
            // 处理多个对象的参数信息(同样的参数，但是需要处理不同的信息，针对的是多个对象的参数)
            var obj = this.adapterOne(id, json, data, type);
